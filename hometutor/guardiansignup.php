<?php
  ...
  if(isset($_POST['register'])){
      ...
      if($password == $c_password) { 
          ...
          if($val == 0){
             $sql = "INSERT INTO guardian(name, city,area ,postal_code, house_no, phone_no, email, password) values('$username','$c_name','$area','$p_code','$h_no','$p_no','$email','$password')";
              mysqli_query($con, $sql);

               
              ?>
                <script>window.alert("You have successfully Create an account.")</script>
             <?php
          ...
          //echo"confirm password not match";
            ?>
                <script>window.alert("Confirm Password Doesn't Match.")</script>
           <?php          
      }
  }
?>
    ...
    <div class="g_form">
       <h1>Guardian Form</h1>
        <form method="post" id="g_signup" >
            ...
            <input type="text" placeholder="house_no" name="h_no">
