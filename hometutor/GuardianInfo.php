<html>
<head>
    
    <style>

        .pen{text-align: center;color: white;display: block;padding-top: 10px;}
        .acc{text-align: center;color: white}
        .re{text-align: center;color: white}
        h3{text-align: center;color: white;margin-top: 80px}
        .copyright{background:#A8C72D;
           text-align:center;
		   padding:15px;
		   font-size:14px;
		   }
            .main_content{background-color: burlywood}		   
            .footer_blocks{padding:20px 0px;overflow:hidden;height: 150px;}
            #footer .footer_blocks .f_block h1{border-bottom:1px solid black;
                     color:black;font-size:30px;text-align: left;}
            .f_block{width:370px;float:left;margin:0px 10px}		 
            #footer .footer_blocks .f_block .links .url_link{list-style:none;}
            #footer .footer_blocks .f_block .links .url_link li{}
            #footer .footer_blocks .f_block .links .url_link li a{color:black;background-color:burlywood ;
                     text-decoration:none;
                     font-size:15px;
                     text-height:5px;}

            #footer .footer_blocks .f_block .links .url_link li a:hover{text-decoration:underline}
        
        body{
         background-color: darkolivegreen;
        }
        .menu{
            width:100%;
            padding-left: 140px;
        }
        ul{
            margin: 0px;
            padding:0px;
            list-style: none;  
        }
        ul li{
            float: left;
            width: 200px;
            height: 40px;
            opacity: .8;
            line-height: 40px;
            text-align: center;
            font-size: 20px;
            background-color: burlywood;
        }
        ul li a{
            text-decoration: none;
            color:black;
            display: block;
        }
        ul li a:hover{
            background-color: gray;
        }
        ul li ul li{
            display: none;
        }
        ul li:hover ul li{
            display: block;
        }
        .logo{
        background-color:#A8C72D;
        background-size: 100px 100px;    
        width: 100%;
        margin-bottom: 10px;    
        }
    </style>
   
</head>
    
<body>
    <haeder>
       <div class="logo">
         <img src="../img/l.jpg"/>
        </div>
        <div class="menu">
         <ul>
             <li><a href="gpage.php">Home</a></li>
             <li><a href="searchtutor.php">SearchTutor</a></li>
             <li><a href="GuardianInfo.php">ShowRequest</a></li>
             <li><a href="rating.php">RatingTutor</a></li>
             <li><a href="Logout.php">Logout</a></li>
         </ul>    
        </div>
      </haeder>


</body>    



<?php
  //connection
  $con=mysqli_connect("localhost", "root", "", "hometutor");

    if($con)
    {
        //echo "ok";
    }
    else{
        die("Connection failed because ".mysqli_connect_error());
    }
    error_reporting(0);
    session_start();
    $gId =  $_SESSION["gId"]; 
    //echo $gId;
    
    //<!--Pending -->
    
      ?>
  
    <!-- Accept -->
   <h3>Pending</h3>
    <?php
      $query="SELECT * FROM bmessage WHERE g_id='$gId' AND pending='yes' ";
     
      $r=mysqli_query($con,$query);
      $val=mysqli_num_rows($r);
      

          if($val>0)
          {      

               while($rows=mysqli_fetch_array($r))
               {
                 ?> 
                 <div class="pen">
                     
                    <?php echo "Day: "?><?php echo $rows['day'];?><br>
                    <?php echo "Tution-Time: "?><?php echo $rows['tutiontime'];?><br>
                    <?php echo "Salary: "?><?php echo $rows['salary'];?><br>  
                     </div>
                  <?php
               }

          } 
          else{
             // echo"There is no One";
         }
?>
  
    <!-- Accept -->
   <h3>Accept</h3>
    <?php 
     $query="SELECT * FROM bmessage WHERE g_id='$gId' AND accept = 'yes' AND pending='no' ";
     
      $r=mysqli_query($con,$query);
      $val=mysqli_num_rows($r);
      

          if($val>0)
          {      
               while($rows=mysqli_fetch_array($r))
               {
                 ?> 
                 <div class="acc">
                     
                    <?php echo "Day: "?><?php echo $rows['day'];?><br>
                    <?php echo "Tution-Time: "?><?php echo $rows['tutiontime'];?><br>
                    <?php echo "Salary: "?><?php echo $rows['salary'];?><br>  
                    <a href='AcceptedTutorInfo.php?t_id=<?php echo $rows['t_id'] ;?>' >Show Tutor </a>
                               
                </div>
                  <?php
               }        
          } 
          else{
              //echo"There is no One";
         }
  
?>     
   <!-- Reject -->
    <h3>Reject</h3>
    <?php 
     $query="SELECT * FROM bmessage WHERE g_id='$gId' AND accept = 'no' AND pending='no' ";
     
      $r=mysqli_query($con,$query);
      $val=mysqli_num_rows($r);
      

          if($val>0)
          {      
               while($rows=mysqli_fetch_array($r))
               {
                 ?> 
                    <div class="re">
                     
                     <?php echo "Day: "?><?php echo $rows['day'];?><br>
                     <?php echo "Tution-Time: "?><?php echo $rows['tutiontime'];?><br>
                     <?php echo "Salary: "?><?php echo $rows['salary'];?><br>  
                     </div>
                  <?php
               }
          } 
          else{
             // echo"There is no One";
         }
?>

   
<footer id="footer">
<div class="main_content">
<div class="footer_blocks">

<div class="f_block">
<h1>Follow-Us</h1>

<div class="links">
<ul class="url_link">
<li><a href="#">FaceBook</a></li>
<li><a href="#">Twitter</a></li>
</ul>
</div>
</div>


<div class="f_block">
<h1>Customer-Service</h1>
<span>PhoneNo:01794747894</span><br/>
<span>Email:halmamun27@gmail.com</span>       
</div>

<div class="f_block">
<h1>Payment</h1>
<span>BkashN0:01794747894</span><br/>
<span>RocketNo:017947478941</span>       
</div>

</div>
</div>

<div class="copyright">
<div class="main_content">
&copy;All rights are reserved by VIT.COM
</div>
</div>
</footer>  
    
</html>    